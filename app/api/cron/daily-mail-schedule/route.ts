...
export async function GET(req: NextRequest) {
  ...
  for (const user of users) {
    ...
    if (!entries || entries.length === 0) continue // No classes tomorrow
    ...
    for (const entry of entries) {
      ...
      const token = generateToken(user.id, entry.id, tomorrowStr)
      const markPresentUrl = `${baseUrl}/api/attendance/mark-email?user_id=${user.id}&course_id=${entry.id}&date=${tomorrowStr}&status=present&token=${token}`
    ...
    const htmlBody = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f9fafb; padding: 24px; border-radius: 12px;">
        <h1 style="color: #111827; font-size: 24px; margin-bottom: 8px; text-align: center;">Tomorrow's Timetable</h1>
        <p style="color: #6b7280; font-size: 16px; text-align: center; margin-bottom: 32px;">${displayDate}</p>
        
        <p style="color: #374151; font-size: 16px; margin-bottom: 24px;">Hi ${user.name.split(' ')[0]}, here is your schedule for tomorrow. You can mark your attendance directly from this email.</p>
