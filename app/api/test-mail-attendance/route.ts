...
export async function GET(req: NextRequest) {
  ...
  if (!entries || entries.length === 0) return NextResponse.json({ message: 'You have no classes scheduled for today.' })
  ...
  for (const entry of entries) {
    ...
    const token = generateToken(userProfile.id, entry.id, todayStr)
    const markPresentUrl = `${baseUrl}/api/attendance/mark-email?user_id=${userProfile.id}&course_id=${entry.id}&date=${todayStr}&status=present&token=${token}`
  ...
  const htmlBody = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f9fafb; padding: 24px; border-radius: 12px;">
      ...
      <p style="color: #374151; font-size: 16px; margin-bottom: 24px;">Hi ${userProfile.name.split(' ')[0]}, here is a test email of your schedule for today. You can mark your attendance directly from this email.</p>
